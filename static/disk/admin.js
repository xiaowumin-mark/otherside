window.onload = function () {
    removeCookie('FilesAddAdmin')
    setCookie('FilesAddAdmin', '')
    ...
    GetFileList('/')
    $.ajax({
        url: ym+"api/town/disk/passppter",      //请求接口的地址
    ...
    $.ajax({
        ...
        success: function (res) {                      //请求成功后的操作
            ...
            for (var i = 0; i < a.length; i++) {
                let y = document.getElementById('ppterlist').innerHTML
                document.getElementById('ppterlist').innerHTML = y + '<tr id="ppterlist' + a[i].ID + '"><th>' + a[i].ID + '</th><td>' + a[i].Uname + '</td><td>' + a[i].Upwd + '</td><td>' + a[i].Uemail + '</td><td><button onclick="deleteppter(' + a[i].ID + ',\'ppterlist' + a[i].ID + '\')"type="button"class="btn btn-secondary">删除</button></td></tr>'
            }
...
function blppter(arr) {
    for (var i = 0; i < arr.length; i++) {
        console.log(arr[i]);
        let a = arr[i]
        let all = '<div id="ppter' + a.ID + '"class="card a"style="width: 18rem;"><div class="card-body"><h5 class="card-title">' + a.Uname + '</h5><h6 class="card-subtitle mb-2 text-muted">等待审核</h6><p class="card-text">用户名：' + a.Uname + '</p><p class="card-text">密码：' + a.Upwd + '</p><p class="card-text">邮箱：' + a.Uemail + '</p><p class="card-text">个人介绍：' + a.Umsg + '</p><button onclick="passppter(' + a.ID + ',\'ppter' + a.ID + '\')" id=""type="button"class="btn btn-primary">通过</button><button onclick="deleteppter(' + a.ID + ',\'ppter' + a.ID + '\')" id=""type="button"class="btn btn-primary">删除</button></div></div>'
        let y = document.getElementById('ppter').innerHTML
        document.getElementById('ppter').innerHTML = y + all
    }
}

function passppter(id, uid) {
    console.log(id);
    $.ajax({
        url: ym+"api/town/disk/passppter",      //请求接口的地址
        type: "GET",                                   //请求的方法GET/POST
...
function deleteppter(id, uid) {
    console.log(id);
    $.ajax({
        url: ym+"api/town/disk/deleteppter",      //请求接口的地址
        type: "GET",                                   //请求的方法GET/POST
        data: { "id": id },
        success: function (res) {                      //请求成功后的操作
            console.log(res);
...
function uploadNewImg() {
    // 构造表单数据
    getCookie('FilesAddAdmin')
    let formData = new FormData();
    // --- 文件
    let file = $("#new-img")[0].files[0];
    formData.append("file", file);
    var add = JSON.stringify({
        "add": getCookie('FilesAddAdmin'),
    });
    //这里包装 可以直接转换成对象
    formData.append('add', getCookie('FilesAddAdmin'))
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.Msg == "true") {
                    ...
                    GetFileList(getCookie('FilesAddAdmin'))
                }
            }
        })
...
function FileHtml(type, name, time, size) {
    ...
    if (num[b] == "png") {
        ...
        let d = '<a onclick="DeleteFile(\''+name+'\')" href="#FileList" class="link-primary">删除</a>'
...
function ShowFile(name, time, size, type) {
    ...
    if (sj == true) {
        ...
        if (getCookie('FilesAddAdmin') == '') {
            ...
            if (type == "pdf") {
                let p = '<iframe src="' + z + getCookie('FilesAddAdmin') + '/' + name + '" width="100%" height="100%" frameborder="0"></iframe>'
                document.getElementById('showfile').innerHTML = p
            } else {
                let p = '<div data-v-093a1112=""><div data-v-093a1112="" class="nd-detail__img bg"><!----><img data-v-093a1112="" src="' + z + getCookie('FilesAddAdmin') + '/' + name + '" class="thumb"><!----></div></div><div data-v-093a1112=""class="nd-detail__props"><div data-v-093a1112=""class="nd-detail__name"><span data-v-093a1112="">' + name + '</span></div><div data-v-093a1112=""class="prop">创建时间:' + time + '</div><div data-v-093a1112=""class="prop">最后修改:' + time + '</div><div data-v-093a1112=""class="prop">文件格式:' + type + '</div><div data-v-093a1112=""class="prop">文件大小:' + size + '</div><div data-v-093a1112=""class="prop"><span data-v-093a1112=""></span><div data-v-59549eba=""data-v-093a1112=""class="nd-file-main__nav simple"><div data-v-59549eba=""class="nd-file-main__nav-left"><div data-v-59549eba=""><!----><span data-v-59549eba=""class="nd-file-selector__nav-item"><span class="nd-file-selector__nav-item-title text-ellip"></span><span class="nd-file-selector__nav-item-sep">&gt;</span></span><span data-v-59549eba=""class="nd-file-selector__nav-item is-disable-nav"><span class="nd-file-selector__nav-item-title text-ellip"></span><span class="nd-file-selector__nav-item-sep">&gt;</span></span></div></div><!----></div></div></div>'
                document.getElementById('showfile').innerHTML = p
            }
        }
...
function OpenFile(add, choose) {
    ...
    if (choose == 'up') {
        ...
        let fileadd = getCookie('FilesAddAdmin')
        console.log(fileadd);
        setCookie('FilesAddAdmin', fileadd + add)
        let fileaddd = getCookie('FilesAddAdmin')
...
function DeleteFile(Name) {
    ...
    $.ajax({
        url: ym+"api/town/disk/deletefile",      //请求接口的地址
        type: "GET",                                   //请求的方法GET/POST
        data: { 'add': add, 'name': Name },
        success: function (res) {                      //请求成功后的操作
            console.log(res);
        ...
        error: function (a) {                       //请求失败后的操作
            console.log(22);
            err('e', '删除文件' + Name + '，失败！')                     //请求失败在控制台输出22
        }
    })
}